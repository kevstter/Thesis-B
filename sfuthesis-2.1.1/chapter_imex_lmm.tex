...
\end{align}
This expression deviates from the exact solution, $\exp(z)$, in the $z^3$ term  and shows a quadratic dependence on $p$. Therefore at coarse step-sizes, sizable $p$ may crater the observed order of accuracy. On the other hand, the methods we have proposed all show only a linear dependence on $p$ under the same analysis.

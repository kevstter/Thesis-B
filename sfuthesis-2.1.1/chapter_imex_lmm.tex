...
\begin{figure}
        \centering
\includegraphics[draft=false,width=0.65\textwidth]{lmm_conv_ammc}
...
First, let us tabulate the observed (non)convergence of SBDF3 at various values of $p$. In \cref{table: sbdf3 nonconvergence} we document a case exhibiting reduction in the order of accuracy and a case where the method diverged as the time step-size was reduced. Divergence of SBDF3, SBDF4, can be attributed to the fact that their parameter restriction leaves only a bounded interval.
...
Combining the parameter restrictions listed in \cref{table:amp poly 34} and the relation \cref{ammc eig}, we have for SBDF3 the parameter restriction
\begin{align}
        \frac{7}{8(1 + (D_1\bar u^n_j)^2)} \leq p \leq \frac{2}{1 + (D_1 \bar u^n_j)^2},
\quad j=1,\dots,N,
\end{align}
and for SBDF4 the parameter restriction
\begin{align} 
 \frac{15}{16(1 + (D_1\bar u^n_j)^2)} \leq p \leq \frac{5}{4(1 + (D_1 \bar u^n_j)^2)},
\quad j=1,\dots,N.
\end{align}
We can equivalently state these as
\begin{align}
       \max_{1\leq j\leq N} \frac{7}{8(1 + (D_1\bar u^n_j)^2)} \leq p \leq \min_{1\leq j\leq N}\frac{2}{1 + (D_1 \bar u^n_j)^2},
\end{align}
and 
\begin{align} 
 \max_{1\leq j\leq N} \frac{15}{16(1 + (D_1\bar u^n_j)^2)} \leq p \leq  \min_{1\leq j\leq N} \frac{5}{4(1 + (D_1 \bar u^n_j)^2)}.
\end{align}

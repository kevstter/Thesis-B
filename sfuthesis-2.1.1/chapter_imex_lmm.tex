...
\caption[Numerical convergence study with IMEX LMMs]{Numerical convergence study to \cref{ammc } with IMEX LMMs. Convergence of EIN is also included for comparison. Test with mCNAB is omitted, but would otherwise overlap with CNAB.}
\label{lmm conv test}
\end{figure}

\subsubsection{Failure with high order IMEX LMMs}
First, let us tabulate the observed (non)convergence of SBDF3 with various values of $p$. 

\begin{table}[htb!]
        \centering
\caption[(Non)convergence of SBDF with various $p$.]{(Non)convergence of SBDF with various $p$.}
\begin{tabular}{lll}
        a
\end{tabular}
\label{tab: sbdf3 nonconvergence}
\end{table}

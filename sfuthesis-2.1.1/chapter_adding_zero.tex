...
\label{mte}
\end{align}
This has obvious parallels to the more general problem \eqref{generic}. The quantity $\lambda$ captures the character of $\N$, and the component $p\lambda u$ represents the linear component that closely resembles $\N(u)$. 

With the modified test equation, \eqref{mte}, we will now discuss the stability properties of three time stepping methods.
...
In \cite{duchemin2014explicit}, Duchemin and Eggers extended the above method to second order using Richardson extrapolation. They referred to their methodology as explicit-implicit null (EIN). 
The amplification factor, $\xi_{EIN}$, can therefore be expressed in terms of $\xi_E$, 
\begin{align}
\xi_{EIN} 
...
\end{align*}
for all $z<0$, thus guaranteeing the numerator is positive. And since the denominator, as we have mentioned before, is positive, $w+2>0$, and thus unconditional stability is guaranteed if $p>2/3$.

...
\end{remark}
\begin{remark}
	What we now understand is that in order to linearly stabilize effectively, we need the ratio of the eigenvalues to meet a specific bound. This bound, however, is specific to the time stepping procedure, assuming a fixed and reasonable choice of a linear operator $\L$, and is met by choosing a sufficiently large value of $p$.  
\end{remark}
\begin{remark}
	Finally, we must mention that this provides us with a simple and widely applicable avenue to selecting $p$ outside of doing a von Neumann analysis, especially as the latter in many cases may be infeasible. As an example, ...
\end{remark}

\section{Numerical Results}
We present in this section some tests to support our claims.

\subsection{Stability contours}
Stability contours plots are shown in \cref{sbdf1 contours,ein contours} as a verification of the earlier analysis of the linearly stabilized semi-implicit Euler scheme and the EIN method. In the first case, we found the parameter restriction to be $p\geq 0.5$, and this is in agreement with \cref{sbdf1 contours}. When we set $p=0.45$, the stable range along the negative real axis is clearly bounded. But at $p=0.50$ and greater, it is stability region includes the entirety of the negative real axis.
\begin{figure}[htb!]
        \centering
\includegraphics[width=0.65\textwidth]{sbdf1_stab_contour2}
\caption{Stability contours of linearly stabilized SBDF1 with various $p$.}
...
\caption{Stability contours of EIN with various $p$.}
\label{ein contours}
\end{figure}

\subsection{Numerical convergence test}
The convergence of linearly stabilized semi-implicit Euler and EIN are demonstrated next on problem \cref{ammc}. We solve to time $T=0.35$ with $N=2048$ spatial grid nodes. As a reference solution, we use an explcit third order Runge-Kutta method with time step-size $\Delta t=\num{1.46e-5}$. This choice of step-size is a result of stability restrictions. As a comparison, with the linearly stabilized schemes, we will solve \cref{ammc} with step-sizes as large as $\Delta t = \num{1.1e-2}$.
...
\caption{Result of numerical convergence study to \cref{ammc}.}
